...
@section('title', 'Edit Laporan - Sistem Laporan')
...
<div class="min-h-screen bg-gray-50">
    ...
    <main class="max-w-4xl mx-auto px-6 py-12">
        ...
        <div class="text-center mb-12">
            <h1 class="text-4xl font-bold text-gray-900 mb-4">Edit Laporan</h1>
        ...
        <!-- Form -->
        <form action="{{ route('reports.update', $report->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
            @csrf
            @method('PUT')
            ...
            <div class="mb-8">
                ...
                @if($report->photo_path)
                    <div class="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                        <p class="text-sm text-blue-900 mb-3">Foto saat ini:</p>
                        <img src="{{ asset('storage/' . $report->photo_path) }}" alt="{{ $report->title }}" class="max-w-xs h-auto rounded-lg border border-blue-200">
                    </div>
                @endif
            ...
            <div class="flex justify-center gap-4 pt-8 border-t border-gray-200">
                <a href="{{ route('reports.show', $report->id) }}" class="px-8 py-3 bg-gray-300 text-gray-900 font-semibold rounded-lg hover:bg-gray-400 transition-all flex items-center gap-2">
                    <i class="fas fa-times"></i> Batal
                </a>
                <button 
                    type="submit" 
                    class="px-8 py-3 bg-gradient-primary text-white font-semibold rounded-lg shadow-primary hover:shadow-primary-lg hover:-translate-y-0.5 transition-all flex items-center gap-2"
                >
